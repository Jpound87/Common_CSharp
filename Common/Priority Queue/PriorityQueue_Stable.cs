...
namespace Common
{
    ...
    {
        #region Identity
        public const String ControlName = nameof(StablePriorityQueue<T>);
        ...
        #region Globals
        private int currentCountNodes;
        private T[] nodes;
        private long totalNodesEnqueued;
        #endregion /Globals
        ...
        public void Enqueue(T node, float priority)
        {
            ...
            if(currentCountNodes >= nodes.Length - 1)
            {
            ...
            node.Priority = priority;
            currentCountNodes++;
            nodes[currentCountNodes] = node;
            node.QueueIndex = currentCountNodes;
            node.InsertionIndex = totalNodesEnqueued++;
            CascadeUp(node);
        }
        #endregion /Enqueue
        ...
        public T Dequeue()
        {
            ...
            T formerLastNode = nodes[currentCountNodes];
            nodes[1] = formerLastNode;
            formerLastNode.QueueIndex = 1;
            nodes[currentCountNodes] = null;
            currentCountNodes--;
        ...
        public void Remove(T node)
        {
            ...
            //If the node is already the last node, we can remove it immediately
            if (node.QueueIndex == currentCountNodes)
            {
                nodes[currentCountNodes] = null;
                currentCountNodes--;
                return;
        ...
        public void Clear()
        {
            Array.Clear(nodes, 1, currentCountNodes);
            currentCountNodes = 0;
        }
        #endregion
        ...
        private void CascadeDown(T node)
        {
            ...
            if (childLeftIndex > currentCountNodes)
            {
                return;
            }
            ...
            if (HasHigherPriority(childLeft, node))
            {
                ...
                if (childRightIndex > currentCountNodes)
                {
            ...
            else if (childRightIndex > currentCountNodes)
            {
                return;
            }
            else
        ...
        public void Resize(int maxNodes)
        {
            ...
            if (maxNodes < currentCountNodes)
            {
                throw new InvalidOperationException("Called Resize(" + maxNodes + "), but current queue contains " + currentCountNodes + " nodes");
            }
#endif

            T[] newArray = new T[maxNodes + 1];
            int highestIndexToCopy = Math.Min(maxNodes, currentCountNodes);
        ...
        {
            IEnumerable<T> e = new ArraySegment<T>(nodes, 1, currentCountNodes);
